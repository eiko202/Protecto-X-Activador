...
:: change KMS Auto renovación schedule, range in minutes: from 15 to 43200
...
:: when you bundle ProyectoX script(s) inside self-extracting program or run it from another command script
:: if the exe pack or the caller script is running as 32-bit (x86) process
:: ProyectoX script(s) will close then relaunch itself using 64-bit (x64 / ARM64) cmd.exe
:: in that case, be advised not to proceed your pack or caller script depending on ProyectoX script(s) closure
...
set "_mO21a=detectado Office 2021 C2R Retail Esta activada"
set "_mO19a=detectado Office 2019 C2R Retail Esta activada"
set "_mO16a=detectado Office 2016 C2R Retail Esta activada"
set "_mO15a=detectado Office 2013 C2R Retail Esta activada"
set "_mO21c=detectado Office 2021 C2R Retail no se pudo convertir a Volume"
set "_mO19c=detectado Office 2019 C2R Retail no se pudo convertir a Volume"
set "_mO16c=detectado Office 2016 C2R Retail no se pudo convertir a Volume"
set "_mO15c=detectado Office 2013 C2R Retail no se pudo convertir a Volume"
set "_mO14c=detectado Office 2010 C2R Retail no es compatible con ProyectoX"
set "_mO14m=detectado Office 2010 MSI Retail no es compatible con ProyectoX"
set "_mO15m=detectado Office 2013 MSI Retail no es compatible con ProyectoX"
set "_mO16m=detectado Office 2016 MSI Retail no es compatible con ProyectoX"
set "_mOuwp=detectado Office 365/2016 UWP no es compatible con ProyectoX"
...
set w7inf=%SystemRoot%\Migration\WTR\ProyectoX.inf
...
set _AUR=0
if exist %_Hook% dir /b /al %_Hook% %_Nul3% || (
  reg query "%IFEO%\%SppVer%" /v VerifierFlags %_Nul3% && (set _AUR=1&set "_dMode=Auto renovacion") || (reg query "%IFEO%\osppsvc.exe" /v VerifierFlags %_Nul3% && (set _AUR=1&set "_dMode=Auto renovacion"))
)
...
if %_AUR% EQU 1 (
call :Cfgbg %_cWht% "               [1] Activar " %_cGrn% "[%_dMode% Mode]"
) else (
call :Cfgbg %_cWht% "               [1] Activar " %_cBlu% "[%_dMode% Mode]"
)
echo.
if %_AUR% EQU 1 (
call :Cfgbg %_cWht% "               [2] Instalacion de renovacion automatica " %_cGrn% "[%_dHook%]"
) else (
echo                [2] Instalacion de renovacion automatica
)
...
echo.
if %_dDbg%==No (
echo                [4] Habilitar modo de depuracion        [%_dDbg%]
) else (
call :Cfgbg %_cWht% "               [4] Habilitar modo de depuracion         " %_cRed% "[%_dDbg%]"
)
if %_dAwin%==Yes (
echo                [5] Ventanas de proceso           [%_dAwin%]
) else (
call :Cfgbg %_cWht% "               [5] Ventanas de proceso          " %_cYel% "[%_dAwin%]"
)
if %_dAoff%==Yes (
echo                [6] Proceso Office            [%_dAoff%]
) else (
call :Cfgbg %_cWht% "               [6] Proceso Office            " %_cYel% "[%_dAoff%]"
)
if %_dArtv%==Yes (
...
if %_dNxt%==No (
if %sub_next% EQU 1 (
call :Cfgbg %_cYel% "               [V] Anular Office C2R vNext " %_cYel% "[%_dNxt%]"
  ) else (
echo                [V] Anular Office C2R vNext [%_dNxt%]
  )
) else (
if %sub_next% EQU 1 (
call :Cfgbg %_cYel% "               [V] Anular Office C2R vNext " %_cRed% "[%_dNxt%]"
  ) else (
echo                [V] Anular Office C2R vNext [%_dNxt%]
  )
)
...
if %_dWXKMS%==Yes (
echo                [X] Saltar Windows KMS38        [%_dWXKMS%]
) else (
call :Cfgbg %_cWht% "               [X] Saltar Windows KMS38        " %_cYel% "[%_dWXKMS%]"
))
echo                %line4%
echo.
echo                    Complementos
...
echo                [S] Crear Carpeta $OEM$
echo                [D] Decodificar archivos binarios incrustados
echo                [E] Activar {External Mode}
echo           %line3%
echo.
if %_NCS% EQU 0 (
popd
...
if %_AUR% EQU 0 (color 1F&set "mode=Manual") else (color 07&set "mode=Auto renovacion")
)
if %Unattend% EQU 0 (
if %_Debug% EQU 0 (title %_title%) else (set "_title=Proyecto X %uivr% : %mode%"&title Proyecto X %uivr% : %mode%)
) else (
echo.
echo Running Proyecto X %uivr%
)
...
set "nEval=Las ediciones de evaluacion no se pueden activar. Instale el sistema operativo Windows completo."
...
if %ActOffice% EQU 0 (echo.&echo La activacion de Office esta DESACTIVADA...)
...
if %ActWindows% EQU 0 (
  ...
  if %SSppHook% EQU 0 (
    ...
    echo.&echo Error: no se pudo verificar la activacion de KMS ID^(s^) for Windows.&echo El servicio sppsvc o SppExtComObjHook.dll no es funcional.&call :CheckWS
    exit /b
    )
  )
)
...
if %winbuild% GEQ 9200 (
  if %OffUWP% EQU 0 (echo.&echo Sin Instaladar 2013-2021 Producto detectado...) else (echo.&echo %_mOuwp%)
  exit /b
  )
if %winbuild% LSS 9200 (if %loc_off14% EQU 0 (echo.&echo Sin Instalar Office %aword% Producto detectado...&exit /b))
)
...
if %WinPerm% EQU 1 (
for /f "tokens=2 delims==" %%x in ('%_qr%') do echo Comprobando: %%x
echo El producto esta activado permanentemente.
exit /b
)
call :insKey
exit /b
...
if %OsppHook% EQU 0 (echo.&echo Sin Instalar Office %aword% Producto detectado...&exit /b)
if %winbuild% GEQ 9200 if %loc_off14% EQU 0 (echo.&echo Sin Instalar Office %aword% Producto detectado...&exit /b)
set err_offsvc=0
...
set "_qr=%_zz7% %spp% %_zz2% %_zz5%ID='%app%'%_zz6% %_zz3% Name %_zz8%"
if %ESU_ADD% EQU 0 for /f "tokens=2 delims==" %%x in ('%_qr%') do echo Activando: %%x
if %ESU_ADD% EQU 1 for /f "tokens=2 delims==f" %%x in ('%_qr%') do echo Activando: %%x
set ESU_ADD=0
set "_qr=wmic path %spp% where ID='%app%' llamar Activar"
if %WMI_VBS% NEQ 0 set "_qr=%_csm% "%spp%.ID='%app%'" Activar"
%_qr% %_Nul3%
call set ERRORCODE=%ERRORLEVEL%
if %ERRORCODE% EQU -1073418187 (
echo Activacion del producto fallida: 0xC004F035
...
echo Activación del producto exitosa, pero el periodo restante no aumento.
if %OSType% EQU Win7 echo Esto podría estar relacionado con el error descrito en KB4487266
exit /b
)
set Act_OK=0
...
echo Activacion del producto exitosa
echo Periodo restante: %gpr2% days ^(%gpr% minutes^)
exit /b
)
cmd /c exit /b %ERRORCODE%
if %ERRORCODE% NEQ 0 (
echo Activacion del producto fallida: 0x!=ExitCode!
) else (
echo Activacion del producto fallida
)
echo Periodo restante: %gpr2% days ^(%gpr% minutes^)
set S_OK=0
exit /b
...
echo.&echo %line3%&echo.
echo Instalacion del emulador KMS local...
)
set "AddExc="
...
if %winbuild% GEQ 9600 (
  %_qr% %_Nul3% && set "AddExc= y exclusion de Windows Defender"
)
if %_verb% EQU 1 (
echo.
echo Agregando archivo%AddExc%...
echo %SystemRoot%\System32\SppExtComObjHook.dll
)
if %_AUR% EQU 1 (
call :StopService sppsvc
...
if %_verb% EQU 1 (
echo.
echo agregando de claves de registro....
)
...
if %winbuild% GEQ 9600 (
  ...
  %_qr% %_Nul3% && set "RemExc= y exclusiones de Windows Defender"
)
if %_verb% EQU 1 (
if %Silent% EQU 0 if %_Debug% EQU 0 (
mode con cols=100 lines=34
...
echo.&echo %line3%&echo.
echo Desinstalacion de KMS local Emulator...
echo.
echo Eliminacion de archivos%RemExc%...
)
...
if %_verb% EQU 1 (
echo.
echo Eliminacion de claves de registro...
)
...
if %_verb% EQU 1 (
echo.
echo Eliminacion de tareas programadas....
echo %_TaskEx%
)
...
echo aplique la activacion manual o de renovacion automatica y no la desinstale despues.
)
if %Unattend% NEQ 0 goto :TheEnd
echo.&echo %line3%&echo.
echo Pulse cualquier tecla para continuar....
...
echo.
echo Agregando tareas programadas...
echo %_TaskEx%
)
goto :eof
...
if not exist "!_oem!\$OEM$\$$\Setup\Scripts\ProyectoX.cmd" mkdir "!_oem!\$OEM$\$$\Setup\Scripts"
copy /y "!_batf!" "!_oem!\$OEM$\$$\Setup\Scripts\ProyectoX.cmd" %_Nul3%
(
echo @echo off
echo call %%~dp0ProyectoX.cmd /s /a
echo cd \
...
:CreateBIN
cls
if exist "!_oem!\ProyectoX-bin\*.dll" if exist "!_oem!\ProyectoX-bin\*.cab" (
echo.&echo %line3%&echo.
echo Binaries Folder already exist...
echo "!_oem!\ProyectoX-bin"
echo.
echo Manually remove it if you wish to create a fresh copy.
echo.&echo %line3%&echo.
echo Press any key to continue...
...
if not exist "!_oem!\ProyectoX-bin\*.dll" mkdir "!_oem!\ProyectoX-bin"
pushd "!_oem!\ProyectoX-bin"
%_Nul3% rmdir /s /q .
setlocal
set "TMP=%SystemRoot%\Temp"
set "TEMP=%SystemRoot%\Temp"
...
echo.
echo Microsoft Office se activa con una licencia vNext.
)
if %sub_proj% EQU 1 (
  for %%a in (%_PrjSKU%) do set _%%a=0
echo.
echo Microsoft Project se activa con una licencia vNext.
)
if %sub_vsio% EQU 1 (
  for %%a in (%_VisSKU%) do set _%%a=0
echo.
echo Microsoft Visio se activa con una licencia vNext.
)
...
if %_C16Msg% EQU 1 (
echo.
echo Conversion de Office C2R minorista a volumen:
)
...
if %GracePeriodRemaining% EQU 0 (
  if %winID% EQU 1 (set "ExpireMsg=La maquina está permanentemente activada..") else (set "ExpireMsg=The product is permanently activated.")
  ) else (
...
echo Active el producto para actualizar los valores de informacion del cliente de KMS.
exit /b
)
