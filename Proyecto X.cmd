...
if exist "%PUBLIC%\proyectx.html" del /f /q "%PUBLIC%\proyectx.html"
...
if %_AUR% EQU 0 (set "_dHook=No instalada") else (set "_dHook=instalada")
...
echo           %line3%
echo.
rem echo                [1] Activar [%_dMode% Modo]
if %_AUR% EQU 1 (
call :Cfgbg %_cWht% "               [1] Activar " %_cGrn% "[%_dMode% Modo]"
) else (
call :Cfgbg %_cWht% "               [1] Activar " %_cBlu% "[%_dMode% Mode]"
)
echo.
if %_AUR% EQU 1 (
call :Cfgbg %_cWht% "               [2] Instalar activacion de renovacion automatica " %_cGrn% "[%_dHook%]"
) else (
echo                [2] Instalar activacion de renovacion automatica
)
...
if %_dAwin%==Yes (
echo                [5] proceso de Windows           [%_dAwin%]
) else (
call :Cfgbg %_cWht% "               [5] proceso de Windows         " %_cYel% "[%_dAwin%]"
)
if %_dAoff%==Yes (
echo                [6] proceso de Office            [%_dAoff%]
) else (
call :Cfgbg %_cWht% "               [6] proceso de Office            " %_cYel% "[%_dAoff%]"
)
if %_dArtv%==Yes (
...
echo                %line4%
echo.
echo                    Diversas Aplicaciones:
echo.
echo                [8] Verificar estado de activacion {vbs}
echo                [9] Verificar estado de activacion {wmi}
echo                [S] Crear Carpeta $OEM$
echo                [D] Decodificar archivos binarios incrustados
echo                [R] Leeme si quieres
echo                [E] Activar modo {External Mode}
echo           %line3%
echo.
if %_NCS% EQU 0 (
popd
)
choice /c 1234567890EDRSVX /n /m ">           Elija una opcion de menu, o presione 0 para salir: "
set _el=%errorlevel%
...
if %Unattend% EQU 0 (
if %_Debug% EQU 0 (title %_title%) else (set "_title=proyectx %uivr% : %mode%"&title proyectx %uivr% : %mode%)
) else (
echo.
echo Running proyectx %uivr%
)
...
if %winbuild% GEQ 9200 (
  if %OffUWP% EQU 0 (echo.&echo No Installed Office 2013-2021 Producto detectado...) else (echo.&echo %_mOuwp%)
  exit /b
  )
if %winbuild% LSS 9200 (if %loc_off14% EQU 0 (echo.&echo No Installed Office %aword% Producto detectado...&exit /b))
)
...
if %OsppHook% EQU 0 (echo.&echo No instalada Office %aword% Producto detectado...&exit /b)
if %winbuild% GEQ 9200 if %loc_off14% EQU 0 (echo.&echo No instalada Office %aword% Producto detectado...&exit /b)
set err_offsvc=0
...
set "_key="
set "_qr=%_zz7% %spp% %_zz2% %_zz5%ID='%app%'%_zz6% %_zz3% Name %_zz8%"
if %ESU_ADD% EQU 0 for /f "tokens=2 delims==" %%x in ('%_qr%') do echo Instalacion de clave: %%x
if %ESU_ADD% EQU 1 for /f "tokens=2 delims==f" %%x in ('%_qr%') do echo Instalacion de clave: %%x
set ESU_ADD=0
call :keys %app%
...
echo Periodo restante: %gpr2% dias ^(%gpr% minutos^)
exit /b
)
cmd /c exit /b %ERRORCODE%
if %ERRORCODE% NEQ 0 (
...
echo.&echo %line3%&echo.
echo Desinstalando el emulador KMS local...
echo.
echo Eliminar archivos%RemExc%...
)
...
echo aplique la activacion manual o de renovacion automatica y no lo desinstale despues.
)
if %Unattend% NEQ 0 goto :TheEnd
echo.&echo %line3%&echo.
echo Pulse cualquier tecla para continuar......
...
echo.
echo Agregando tarea programada...
echo %_TaskEx%
)
goto :eof

:CreateReadMe
if not exist "%PUBLIC%\proyectx.html" (
pushd %PUBLIC%
%_Nul3% %_psc% "$f=[IO.File]::ReadAllText('!_batp!') -split ':readme\:.*'; [IO.File]::WriteAllText('proyectx.html',$f[1].Trim(),[System.Text.Encoding]::UTF8)"
if %Unattend% EQU 0 title %_title%
popd
)
if exist "%PUBLIC%\proyectx.html" start "" "%PUBLIC%\proyectx.html"
timeout /t 2 %_Nul3%
goto :eof
...
if not exist "!_oem!\$OEM$\$$\Setup\Scripts\proyectx.cmd" mkdir "!_oem!\$OEM$\$$\Setup\Scripts"
copy /y "!_batf!" "!_oem!\$OEM$\$$\Setup\Scripts\proyectx.cmd" %_Nul3%
(
echo @echo off
echo call %%~dp0proyectx.cmd /s /a
echo cd \
...
:CreateBIN
cls
if exist "!_oem!\proyectx-bin\*.dll" if exist "!_oem!\proyectx-bin\*.cab" (
echo.&echo %line3%&echo.
echo Binaries Folder already exist...
echo "!_oem!\proyectx-bin"
echo.
echo Manually remove it if you wish to create a fresh copy.
echo.&echo %line3%&echo.
echo Pulse cualquier tecla para continuar......
...
if not exist "!_oem!\proyectx-bin\*.dll" mkdir "!_oem!\proyectx-bin"
pushd "!_oem!\proyectx-bin"
%_Nul3% rmdir /s /q .
setlocal
set "TMP=%SystemRoot%\Temp"
set "TEMP=%SystemRoot%\Temp"
...
if %_C16Msg% EQU 1 (
echo.
echo convirtiendo Office C2R Retail-to-Volume:
)
...
<head>
<div id="shit">
  <a href="http://knowyourmeme.com/">NICE MEME</a>
</div>


<div id="meme2" class="meme">
 <img style="width: 13%;" src="http://www.clipartbest.com/cliparts/niB/GkB/niBGkBRXT.png"/>
</div>

<div id="meme3" class="meme">
 <img style="width: 11%;" src="http://vignette4.wikia.nocookie.net/meme/images/3/3d/LOLGuy.png/revision/latest?cb=20110821045825"/>
</div>

<div id="meme4" class="meme">
 <img style="width: 11%;" src="https://i.imgur.com/mjhjsZP.jpg"/>
</div>

<div id="meme5" class="meme">
 <img style="width: 12%;" src="http://fc08.deviantart.net/fs70/f/2012/123/2/9/meme_happy_010_by_sofidesiings-d4yfc0r.png"/>
</div>

<div id="meme6" class="meme">
 <img style="width: 17%;" src="http://i0.kym-cdn.com/photos/images/newsfeed/000/103/740/Me%20Gusta.png?1318992465"/>
</div>

<div id="meme7" class="meme">
 <img style="width: 20%;" src="https://pa1.aminoapps.com/6299/252755d23476b7714cd3cd96d8a3345dd1165771_hq.gif"/>
</div>

<div id="meme8" class="meme">
 <img style="width: 25%;" src="http://vignette1.wikia.nocookie.net/lego/images/f/f2/Hd_computer_guy_meme_by_zapgod16-d4t2jh3.png/revision/latest?cb=20130322044600"/>
</div>

<div id="meme9" class="meme">
 <img style="width: 20%;" src="http://fc03.deviantart.net/fs70/i/2011/289/7/5/yao_ming_heck_no_by_rober_raik-d4czlxk.png"/>
</div>

<div id="meme10" class="meme">
 <img style="width: 22%;" src="https://static.wikia.nocookie.net/memes-pedia/images/7/7c/Vaca.gif/revision/latest?cb=20210207232718&path-prefix=es"/>
</div>

<div id="meme11" class="meme">
 <img style="width: 17%;" src="https://4.bp.blogspot.com/-xDEbPadF_hk/UzxFWrn-O5I/AAAAAAAAW8U/wY7FMrqFTpw/s1600/meme_concentrated.png"/>
</div>

<div id="meme12" class="meme">
 <img style="width: 15%;" src="https://i.imgur.com/5lIeJ.png"/>
</div>

<div id="meme13" class="meme">
 <img style="width: 9%;" src="http://cdn-frm-us.wargaming.net/wot/us/uploads/monthly_03_2014/post-374071-0-84934000-1393999636.png"/>
</div>
